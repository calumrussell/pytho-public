...
import {
  writeGraph as writeTimeGraph,
  reducer as timeReducer,
  init as timeInit,
} from "./components/reducers/time";
import {
  writeGraph as writeHistGraph,
  reducer as histReducer,
  init as histInit,
} from "./components/reducers/histogram";
import {
  writeGraph as writeBarGraph,
  reducer as barReducer,
  init as barInit,
} from "./components/reducers/bar";
import {
  writeGraph as writeStackedBarGraph,
  reducer as stackedBarReducer,
  init as stackedBarInit,
} from "./components/reducers/stackedbar";
import { TimeButtons } from "./components/timebuttons";
import { LogButton } from "./components/logbutton";
import { ButtonRow } from "./style";

export const LineChart = ({ xValues, yValues, labels, rootId }) => {
...
export const LineChartWithTimeButtons = ({
  ...
  const [state, dispatch] = useReducer(timeReducer, initState, timeInit);
...
export const HistogramChart = ({ values, rootId, runs }) => {
  const initState = {
    ref: createRef(),
    rootId,
    data: {
  ...
  const [state, dispatch] = useReducer(histReducer, initState, histInit);
...
export const BarChart = ({ values, labels, rootId }) => {
  const initState = {
    ref: createRef(),
    rootId,
    data: {
  ...
  const [state, dispatch] = useReducer(barReducer, initState, barInit);
...
export const StackedBarChart = ({ xValues, yValues, labels, rootId }) => {
  const stackFunc = stack().keys(labels);
