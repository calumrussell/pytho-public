import { writeReturn, updateReturn } from "../../return";
import { addButtonHook, timeButtonUpdater } from "../../timebuttons";
import { addButtonHook as addLogButtonHook } from "../../logbutton";
import { axisBuilder, updateAxis, writeAxis } from "../../axis";
import { lineBuilder, updateLine, writeLine } from "../../element";
...
const findPositionsInValues = (xAxis, xValues, xGetter) => {
  return [
    xAxis.findIndex((d) => xGetter(d).getTime() >= xValues[0].getTime()),
    xAxis.findIndex((d) => xGetter(d).getTime() >= xValues[1].getTime()),
  ];
};

const filterValuesFromPositions = (xAxis, yAxis, positions) => {
  const [p0, p1] = positions;
  return [xAxis.slice(p0, p1), yAxis.map((row) => row.slice(p0, p1))];
};
...
export const reducer = (state, action) => {
  switch (action.type) {
    ...
    case "logButtonPress": {
      ...
      if (!state.isLog) {
        const [, logXValues, logYValues] = logFunc(state, x, y);
    ...
    case "timeButtonPress": {
      const { x, y } = state.data;
      const { newSelection } = timeButtonUpdater(action.period, x, y, state);

      const { xGetter } = state.data;
      ...
      let newLine = undefined;
      if (state.isLog) {
        const [, logXValues, logYValues] = logFunc(state, xValues, yValues);
        newLine = updateChart(state, logXValues, logYValues);
      } else {
