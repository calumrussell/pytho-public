...
export const writeLine = (chartState, line) => {
  const {
    invariants: { rootWrapper, colours },
    data: { y },
  } = chartState;
...
export const lineBuilder = (chartState, axis) => {
  const { xGetter, yGetter, x: xValues } = chartState.data;
  const [x, y] = axis;
...
export const writeRect = (chartState, axis) => {
  ...
  const {
    ...
    data: { xGetter, yGetter, x: xValues, y: yValues },
  } = chartState;

  const translateFunc = (d) => `translate(0, -2)`;
