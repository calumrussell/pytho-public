...
import { TimeButton } from "./style.js";

export const addButtonHook = (dispatch) => {
  selectAll(".chart-timebutton-element").on("click", (e) =>
    dispatch({
...
export const timeButtonUpdater = (period, xValues, yValues, chartState) => {
  const { xGetter } = chartState.data;

  const monthlyTradingDays = 21;
  const lastNDays = (days) => xValues.slice(Math.max(xValues.length - days, 1));
...
export const TimeButtons = (props) => {
  ...
  return (
    <>
      {titles.map((t, i) => (
        <TimeButton className="chart-timebutton-element" name={t} key={i}>
          {t}
        </TimeButton>
      ))}
    </>
