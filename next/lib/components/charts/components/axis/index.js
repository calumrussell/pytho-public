...
import { extent, max, min, merge } from "d3-array";
...
export const axisPointBuilder = (chartState) => {
  ...
  const yAxis = scaleLinear().domain([minYVal, maxYVal]).range([height, 0]);
  return [xAxis, yAxis];
};
...
export const axisStackBuilder = (chartState) => {
  ...
  const yAxis = scaleLinear().domain([0, maxYVal]).range([bottomMargin, 0]);
...
export const axisBuilder = (chartState) => {
  ...
  const xAxis = scaleTime().domain(extent(x, xGetter)).range([0, width]);
...
export const stagedAxisBuilder = (chartState) => {
  ...
  const xAxis = scaleLinear().domain(extent(x, xGetter)).range([0, width]);
  ...
  const yAxisBuilder = (chartState, yValues, yGetter, setMin = 0) => {
    ...
    return scaleLinear().domain([0, maxYVal]).range([bottomMargin, 0]);
  };
  return [xAxis, yAxisBuilder];
};
