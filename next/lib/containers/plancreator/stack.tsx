...
export const Stack = ({ dispatch }: StackProps) => {
  const { errorMessage } = useMessage();

  const stackTypes = ["Gia", "Isa", "Sipp"];

  const [stackType, setStackType] = useState(stackTypes[0]);
  ...
  return (
    <ComponentWrapper>
      ...
      <FormWrapper>
        <FormLabel htmlFor="metis-stack-type">
        ...
        />
        <FormLabel htmlFor="metis-stack-value">
        ...
        />
        <Button onClick={addButtonClick}>Add Stack</Button>
      </FormWrapper>
    </ComponentWrapper>
  );
};
