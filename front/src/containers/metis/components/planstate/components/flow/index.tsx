...
export const Flow = () => {
  ...
  if (plan != undefined) {
    ...
    const incomeTypeGrowthOptions = [
      'None',
      'Static',
    ];

    const expenseTypeGrowthOptions = [
      'None',
      'Inflation-linked',
    ];
    ...
    const [
      growthOption,
      setGrowthOption,
    ] = useState(incomeTypeGrowthOptions[0]);
    // Defaults to false, but is only used for Expense
    const [
      isInflationLinked,
      setIsInflationLinked,
    ] = useState(false);
    // Not user-controlled
    const [
      growthOptions,
      setGrowthOptions,
    ] = useState(incomeTypeGrowthOptions);
    ...
    const addButtonClick = (ev: React.MouseEvent<HTMLButtonElement>) => {
      ...
      const flow = {
        person,
        schedule: {
          schedule_type: ScheduleType.EndOfMonth,
        },
      };
      ...
      if (flowType == 'Expense' && pct != undefined) {
        updateFlow({
          ...flow,
          pct: pct,
          flow_type: FlowType.PctOfIncomeExpense,
        });
        resetOptions();
        return;
        ...
        updateFlow({
          ...flow,
          value: flowValue,
          flow_type: FlowType.InflationLinkedExpense,
        });
        resetOptions();
        return;
      ...
      if (flowType == 'Employment' && isPaye && growthOption === 'Static') {
        updateFlow({
          ...
          value: flowValue,
          flow_type: FlowType.EmploymentPAYEStaticGrowth,
        });
        resetOptions();
        return;
        ...
        updateFlow({
          ...
          static_growth: staticGrowth,
          value: flowValue,
          flow_type: FlowType.EmploymentStaticGrowth,
        });
        resetOptions();
        return;
        ...
        updateFlow({
          ...flow,
          value: flowValue,
          flow_type: FlowType.EmploymentPAYE,
        });
        resetOptions();
        return;
    ...
    return (
      <ComponentWrapper>
        <FormWrapper>
          ...
          <Nullable
            condition={ flowType === 'Employment' }>
            <FormLabel
              htmlFor="metis-flow-isPaye">
              <Text
                light>
                Paye
              </Text>
          ...
          <Nullable
            condition={ flowType === 'Expense' }>
            <FormLabel
              htmlFor="metis-flow-isPct">
              <Text
                light>
                Fixed % of Monthly Income
              </Text>
            </FormLabel>
            <FormInput
              id="metis-flow-isPct"
          ...
          <Nullable
            ...
            <FormLabel
              htmlFor="metis-flow-growth-options">
              <Text
                light>
                Growth
              </Text>
            ...
            <Nullable
              condition={ growthOption == 'Static' }>
              <FormLabel
                htmlFor="metis-flow-growth">
                <Text
                  light>
                  Growth % (i.e. 0.05)
                </Text>
          ...
          <Nullable
            condition={ isPctOfIncome === true }>
            <FormLabel
              htmlFor="metis-flow-pct">
              <Text
                light>
                Pct % (i.e. 0.05)
              </Text>
